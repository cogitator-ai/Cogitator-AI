...
describe('agentRoutes', () => {
  ...
  it('GET /agents lists registered agents with tools', async () => {
    ...
    expect(body.agents[0].name).toBe('writer');
  ...
  it('POST /agents/:name/run returns 404 for unknown agent', async () => {
    const app = buildApp();
    const res = await app.request('/agents/ghost/run', json({ input: 'hi' }));
  ...
  it('POST /agents/:name/run returns 400 when input is missing', async () => {
    ...
    const res = await app.request('/agents/bot/run', json({}));
  ...
  it('POST /agents/:name/run returns 400 for invalid JSON', async () => {
    ...
    const res = await app.request('/agents/bot/run', {
      method: 'POST',
      body: 'not json{{{',
      headers: { 'Content-Type': 'application/json' },
    });
  ...
  it('POST /agents/:name/run returns agent output on success', async () => {
    ...
    const res = await app.request('/agents/bot/run', json({ input: 'hello' }));
    expect(res.status).toBe(200);
    const body = await res.json();
    expect(body.output).toBe('response text');
  ...
  it('POST /agents/:name/run forwards context and threadId', async () => {
    ...
    await app.request(
      '/agents/bot/run',
      json({ input: 'hi', context: { key: 'val' }, threadId: 'my-thread' })
    );
...
describe('threadRoutes', () => {
  it('GET /threads/:id returns 503 when memory not configured', async () => {
    ...
    expect(body.error.message).toContain('Memory not configured');
  });
  ...
  it('GET /threads/:id uses Date.now() for empty thread', async () => {
    ...
    const res = await app.request('/threads/empty');
    ...
    expect(body.updatedAt).toBeLessThanOrEqual(after);
  });
  ...
  it('POST /threads/:id/messages returns 503 when memory not configured', async () => {
    const app = buildApp();
    const res = await app.request('/threads/t1/messages', json({ role: 'user', content: 'hello' }));
    expect(res.status).toBe(503);
    const body = await res.json();
    expect(body.error.code).toBe('UNAVAILABLE');
  });

  it('POST /threads/:id/messages returns 400 for invalid JSON', async () => {
    const memory = mockMemory();
    const runtime = mockRuntime({ memory });
    const app = buildApp({
      cogitator: runtime as unknown as CogitatorAppOptions['cogitator'],
    ...
    const res = await app.request('/threads/t1/messages', {
      method: 'POST',
      body: '{broken',
      headers: { 'Content-Type': 'application/json' },
    });
  ...
  it('POST /threads/:id/messages returns 400 when role is missing', async () => {
    ...
    const res = await app.request('/threads/t1/messages', json({ content: 'hello' }));
    expect(res.status).toBe(400);
    const body = await res.json();
    expect(body.error.code).toBe('INVALID_INPUT');
  ...
  it('POST /threads/:id/messages returns 400 when content is missing', async () => {
    ...
    const res = await app.request('/threads/t1/messages', json({ role: 'user' }));
    expect(res.status).toBe(400);
    const body = await res.json();
    expect(body.error.code).toBe('INVALID_INPUT');
  });
...
describe('toolRoutes', () => {
  ...
  it('GET /tools lists tools from agents', async () => {
    ...
    expect(body.tools[0].description).toBe('Tool search');
...
describe('workflowRoutes', () => {
  ...
  it('GET /workflows lists registered workflows', async () => {
    ...
    expect(body.workflows[0].name).toBe('pipeline');
    expect(body.workflows[0].entryPoint).toBe('start');
    expect(body.workflows[0].nodes).toEqual(['start', 'end']);
  });
});
...
describe('swarmRoutes', () => {
  it('GET /swarms returns empty list when no swarms', async () => {
    ...
    const res = await app.request('/swarms');
    expect(res.status).toBe(200);
    const body = await res.json();
  ...
  it('GET /swarms lists swarms with supervisor and workers', async () => {
    ...
    expect(body.swarms[0].name).toBe('team');
    expect(body.swarms[0].strategy).toBe('hierarchical');
    expect(body.swarms[0].agents).toEqual(['boss', 'w1', 'w2']);
  });
